...
Также на сайтe есть поиск.
_Поиск_ происходит по модели books, идет не строгое сравнение(без учета регистра) по  трем полям: title,content или имя автора.
_Сортировка_ используется с помощью sort=asc(дешевле) and sort=desc(дороже), сортировка этих книг идет по убыванию, от наибольшего к наименьшему.
_Новинки_, появляются засчет добавления галочки в базе данных isnew=Yes,No.
_Популярные книги_, появляются с помощью счетчика просмотра добавленый в базу данных:
...
Последние новости отсортированные по дате по убыванию
Блог 5 Новинки
